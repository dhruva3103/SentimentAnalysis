...
tweets = pd.read_csv('/home/lp-new-12/Documents/sentiment_analysis.csv')
# tweets.head(20)
...
class TweetClassifier(object):
    ...
    def calc_TF_IDF(self):
        ...
        for word in self.tf_depressive:
            self.prob_depressive[word] = (self.tf_depressive[word]) * math.log(
        ...
        for word in self.tf_positive:
            self.prob_positive[word] = (self.tf_positive[word]) * math.log((self.depressive_tweets + self.positive_tweets) \
    ...
    def classify(self, processed_message):
        ...
        for word in processed_message:
            if word in self.prob_depressive:
                pDepressive += math.log1p(self.prob_depressive[word])
            else:
                if self.method == 'tf-idf':
                    pDepressive -= math.log1p(self.sum_tf_idf_depressive + len(list(self.prob_depressive.keys())))
                else:
                    pDepressive -= math.log1p(self.depressive_words + len(list(self.prob_depressive.keys())))
            if word in self.prob_positive:
                pPositive += math.log1p(self.prob_positive[word])
            else:
                if self.method == 'tf-idf':
                    pPositive -= math.log1p(self.sum_tf_idf_positive + len(list(self.prob_positive.keys())))
                else:
                    # pass
                    pPositive -= math.log1p(self.positive_words + len(list(self.prob_positive.keys())))
            pDepressive += math.log1p(self.prob_depressive_tweet)
            pPositive += math.log1p(self.prob_positive_tweet)
